               ...
               <nav class="navigation navbar navbar-expand-md navbar-dark ">
                  ...
                  <div class="collapse navbar-collapse" id="navbarsExample04">
                     <ul class="navbar-nav mr-auto">
                        ...
                        <li class="nav-item">
                           <a class="nav-link" href="quantrivien_lichthi.php">Kỳ thi</a>
                        </li>
      ...
      <div class="container">
         <section class="content bgcolor-4">
            <form action="" method="post">
               <h1>THÔNG TIN NGƯỜI DÙNG <span class="blue_light"><?php echo $p->laycot("SELECT TenKH FROM khoahoc WHERE MaKH = '$laymakh' LIMIT 1"); ?></span></h1>
               <span class="input input--ruri">
                  <input class="input__field input__field--ruri" placeholder="Mã Khóa Học" readonly type="text" id="makh" name="makh" value="<?php echo $p->laycot("SELECT MaKH FROM khoahoc WHERE MaKH = '$laymakh' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-26">
                  <span  id="mkh" style="color: red; float: right;"></span>
                  </label>
               </span>
               <span class="input input--ruri">
                  <input class="input__field input__field--ruri" placeholder="Tên khóa học" type="text" id="tenkhoahoc" name="tenkhoahoc" value="<?php echo $p->laycot("SELECT TenKH FROM khoahoc WHERE MaKH = '$laymakh' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-27">
                  <span  id="tkh" style="color: red; float: right;"></span>
                  </label>
               </span>
               <span class="input input--ruri">
                  <select class="input__field input__field--ruri" id="trangthai" style="background-color: RGB(47, 50, 56); color: white" name="trangthai">
                     <?php
                     $p->load_selectiontrangthai("SELECT * FROM trangthaikhoahoc");
                     ?>
                  </select>
                  <label class="input__label input__label--ruri" for="input-27">
               ...
               <span class="input input--ruri">
                  <input class="input__field input__field--ruri" placeholder="Ngày bắt đầu" type="date" id="ngaybatdau" name="ngaybatdau" value="<?php echo $p->laycot("SELECT NgayBD FROM khoahoc WHERE MaKH = '$laymakh' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-26">
                  <span  id="nbd" style="color: red; float: right;"></span>
                  </label>
               </span>
               <span class="input input--ruri">
                  <input class="input__field input__field--ruri" placeholder="Ngày kết thúc" type="date" id="ngayketthuc" name="ngayketthuc" value="<?php echo $p->laycot("SELECT NgayKT FROM khoahoc WHERE MaKH = '$laymakh' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-27">
            ...
            switch ($_POST['btn-basic-form'] ?? null) {
               case 'Sửa thông tin': {
                ...
                $ngayketthuc = $_REQUEST['ngayketthuc'];
                     if ($makh != '' && $tenkhoahoc != '' && $trangthai != '' && $ngaybatdau != '' && $ngayketthuc != '') {
                        if ($p->themxoasua("UPDATE khoahoc SET MaKH = '$makh', TenKH = '$tenkhoahoc', Trangthai = '$trangthai', NgayBD = '$ngaybatdau', NgayKT = '$ngayketthuc' WHERE MaKH = '$laymakh' LIMIT 1") == 1) {
                              echo '<script> alert("Cập nhật thông tin khóa học thành công");</script>';
                              header("refresh:0;url=quantrivien_khoahoc.php");
                        } else {
                           echo '<script> alert("Cập nhật thông tin không thành công. Vui lòng kiểm tra lại thông tin khóa học");</script>';
                        }
                     } else {
                        echo '<script> alert("Vui lòng nhập đầy đủ thông tin khóa học để cập nhật");</script>';
                     }
                     break;
                  }
            }
