...
<head>
   ...
   <!-- site metas -->
   <title>ITC | Quản Trị Viên | Lịch Thi</title>
      ...
      <div class="container">
         <section class="content bgcolor-4">
            <form action="" method="post">
               <h1>THÔNG TIN LỊCH THI</h1>
               <span class="input input--ruri">
               <input class="input__field input__field--ruri" placeholder="Mã Kỳ Thi" readonly type="text" id="malichthi" name="malichthi" value="<?php echo $p->laycot("SELECT MaLT FROM lichthi WHERE MaLT = '$laymalt' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-26">
                  </label>
               </span>
               <span class="input input--ruri">
                  <input class="input__field input__field--ruri" placeholder="Tên Lớp Học" type="text" id="lophoc" name="lophoc" value="<?php echo $p->laycot("SELECT TenLopHoc FROM lichthi WHERE MaLT = '$laymalt' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-26">
                  <span  id="tlh" style="color: red; float: right;"></span>
                  </label>
               </span>
               <span class="input input--ruri">
                  <select class="input__field input__field--ruri" id="phongthi" style="background-color: RGB(47, 50, 56); color: white" name="phongthi">
                     <option value="<?php echo $p->laycot("SELECT PhongThi FROM lichthi WHERE MaLT = '$laymalt' LIMIT 1"); ?>"><?php echo $p->laycot("SELECT PhongThi FROM lichthi WHERE MaLT = '$laymalt' LIMIT 1"); ?></option>
                     <?php
               ...
               <span class="input input--ruri">
                  <input class="input__field input__field--ruri" placeholder="Sỉ số" type="number" id="siso" name="siso" value="<?php echo $p->laycot("SELECT SoLuongThiSinh FROM lichthi WHERE MaLT = '$laymalt' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-26">
                  </label>
               </span>
               <span class="input input--ruri">
                  <input class="input__field input__field--ruri" placeholder="Ngày thi" type="date" id="ngaythi" name="ngaythi" value="<?php echo $p->laycot("SELECT NgayThi FROM lichthi WHERE MaLT = '$laymalt' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-26">
                  <span  id="nbd" style="color: red; float: right;"></span>
                  </label>
               </span>
               <span class="input input--ruri">
                  <input class="input__field input__field--ruri" placeholder="Ngày kết thúc" type="time" id="giothi" name="giothi" value="<?php echo $p->laycot("SELECT GioThi FROM lichthi WHERE MaLT = '$laymalt' LIMIT 1"); ?>" />
                  <label class="input__label input__label--ruri" for="input-26">
            ...
            switch ($_POST['btn-basic-form'] ?? null) {
               case 'Sửa thông tin': {
                     $malichthi = $_REQUEST['malichthi'];
                     $phongthi = $_REQUEST['phongthi'];
                     $lophoc = $_REQUEST['lophoc'];
                     $siso = $_REQUEST['siso'];
                     $ngaythi = $_REQUEST['ngaythi'];
                     $giothi = $_REQUEST['giothi'];
                     if ($malichthi != '' && $phongthi != '' && $lophoc != '' && $siso != '' && $giothi != '' && $ngaythi != '') {
                        if ($p->themxoasua("UPDATE lichthi SET MaLT = '$malichthi', TenLopHoc = '$lophoc', PhongThi = '$phongthi', GioThi = '$giothi', NgayThi = '$ngaythi', SoLuongThiSinh = '$siso' WHERE MaLT = '$laymalt' LIMIT 1") == 1) {
                           echo '<script> alert("Cập nhật thông tin lịch thi thành công");</script>';
                           header("refresh:0;url=quantrivien_lichthi.php");
                        } else {
                           echo '<script> alert("Cập nhật thông tin không thành công. Vui lòng kiểm tra lại thông tin lịch thi");</script>';
                        }
                     } else {
                        echo '<script> alert("Vui lòng nhập đầy đủ thông tin lịch thi để cập nhật");</script>';
                     }
                     break;
                  }
            }
